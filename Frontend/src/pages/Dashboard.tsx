...
export function Dashboard() {
    ...
    const eventsCardCopy: Record<UserRole, string> = {
        Administrador:
            "Centralize inscrições, programação, capacidade e documentos em um único fluxo.",
        Secretaria:
            "Centralize inscrições, programação, capacidade e documentos em um único fluxo.",
        Professor: "Acompanhe as inscrições e gerencie os eventos da sua instituição.",
        Aluno: "Inscreva-se em eventos e acompanhe suas participações.",
    };

    const accessCardCopy: Record<"Administrador" | "Secretaria" | "Professor", string> = {
        Administrador:
                    ...
                    <div className="flex items-start gap-4">
                        ...
                        <div className="flex-1">
                            ...
                            <p className="mt-1 text-sm leading-5 text-slate-500">
                                {user?.role ? eventsCardCopy[user.role] : eventsCardCopy.Aluno}
                            </p>
                            <Link
                                to="/events"
                    ...
                    <section className="surface-card p-6">
                        <div className="flex items-start gap-4">
                            ...
                            <div className="flex-1">
                                ...
                                <p className="mt-1 text-sm leading-5 text-slate-500">
                                    {accessCardCopy[user?.role as "Administrador" | "Secretaria" | "Professor"]}
                                </p>
                                <Link
                                    to="/users"
                                    className="mt-4 inline-flex items-center gap-1.5 text-xs font-semibold text-[#109bc6] hover:gap-2">
            ...
            <div className="grid gap-4 lg:grid-cols-[1.4fr_1fr]">
                <section className="surface-card overflow-hidden">
                    ...
                    <div className="divide-y divide-slate-100">
                        {linksWithDescriptions.slice(0, 3).map((item) => (
                            <Link
                                key={item.path}
                                to={item.path}
