...
export const Users: React.FC = () => {
    ...
    useEffect(() => {
        Promise.all([
            ...
            .then(([userResponse, institutionResponse, courseResponse]) => {
    ...
    const filteredUsers = useMemo(
        () =>
            users.filter((user) => {
                ...
                return matchesSearch && (status === 'all' || user.approvalStatus === status);
            }),
        [users, search, status]
    );
    ...
    const setApproval = async (user: ManagedUser, approvalStatus: 2 | 3) => {
        ...
        try {
            ...
            setError(err.response?.data?.message || 'Não foi possível alterar a aprovação.');
        }
    };
    ...
    const toggleActive = async (user: ManagedUser) => {
        ...
        try {
            ...
            setError(err.response?.data?.message || 'Não foi possível alterar a situação.');
        }
    };
            ...
            <div className="mx-auto max-w-7xl">
                ...
                <div className="mb-4 flex flex-col gap-3 rounded-xl border border-slate-200 bg-white p-4 sm:flex-row">
                    ...
                    <select
                        value={status}
                        onChange={(e) =>
                            setStatus(e.target.value === 'all' ? 'all' : (Number(e.target.value) as ApprovalStatus))
                        }
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {filteredUsers.map((user) => (
                                <tr key={user.id} className="hover:bg-slate-50">
                                    ...
                                    <td className="px-4 py-3 text-slate-600">{user.uniqueIdentifier}</td>
                                    <td className="px-4 py-3">
                                        <span
                                            className={`rounded-full px-2.5 py-1 text-xs font-semibold ${user.approvalStatus === 2
                                    ...
                                    <td className="px-4 py-3 text-slate-600">
                                        ...
                                        <div className="text-xs text-slate-400">{formatDate(user.approvalDate)}</div>
                                    </td>
                                    <td className="px-4 py-3">
                                    ...
                                    <td className="px-4 py-3">
                                        <div className="flex justify-end gap-1">
                                            ...
                                            >
                                                {user.isActive ? <CircleOff className="h-4 w-4" /> : <Check className="h-4 w-4" />}
                                            </button>
                                            <button
                                                title="Editar"
                        ...
                        <form onSubmit={submit} className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                            ...
                            <label className="text-sm font-medium text-slate-700">
                                ...
                                <input
                                    ...
                                    onChange={(e) => setForm({ ...form, uniqueIdentifier: e.target.value })}
                                    className="mt-1 w-full rounded-lg border border-slate-300 p-2.5"
                                />
                            </label>
                            <label className="text-sm font-medium text-slate-700">
